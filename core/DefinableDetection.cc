...
*/
void DefinableDetection::sortSelectorsOccDesc(Solver &solver,
                                              vec<Lit> &selectors,
                                              vec<Lit> &assums, int initNbVar,
                                              vec<vec<Lit>> &cls) {
  coupleLit occList[initNbVar], fitSelector[selectors.size()];

  // we don't care the positive phase ... we save the lit in the same place as
  // the var
  for (int i = 0; i < initNbVar; ++i)
    occList[i] = {mkLit((initNbVar << 1) + i, false), 0};

  for (int i = 0; i < cls.size(); i++) {
    vec<Lit> &cl = cls[i];
    for (int j = 0; j < cl.size(); j++)
      occList[var(cl[j])].nbOcc++;
  }
  ...
  for (int i = 0; i < selectors.size(); ++i)
    assums.push(fitSelector[i].l);
} // sortSelectorsOccDesc

void DefinableDetection::sortSelectorsTautologyGeneration(Solver &solver,
                                                          vec<Lit> &selectors,
                                                          int initVarNum,
                                                          vec<Lit> &assums,
                                                          vec<vec<Lit>> &cls) {
  ...
  for (int i = 0; i < (initVarNum << 1); ++i) {
    ...
    for (int j = 0; j < initVarNum << 1; ++j)
      associatedWith[i][j] = 0;
  }
  ...
  for (int clIndex = 0; clIndex < initClauses.size(); ++clIndex) {
    ...
    for (int litIndex = 0; litIndex < cl.size(); ++litIndex) {
      for (int litIndex2 = 0; litIndex2 < cl.size(); ++litIndex2) {
        if (litIndex == litIndex2)
          continue;
        int associatedWithIndex = sign(cl[litIndex]) == false
                                      ? (var(cl[litIndex]) << 1)
                                      : 1 + (var(cl[litIndex]) << 1);
        int associatedWithLiteralIndex = sign(cl[litIndex2]) == false
                                             ? (var(cl[litIndex2]) << 1)
                                             : 1 + (var(cl[litIndex2]) << 1);
  ...
  for (int i = 0; i < initVarNum; ++i) {
    ...
    for (int j = 0; j < initVarNum; ++j) {
      if (i == j)
        continue;
      tauts[(i << 1) + 1] = associatedWith[(i << 1)][(j << 1)] *
                            associatedWith[(i << 1) + 1][(j << 1) + 1];
      tauts[(i << 1) + 1] += associatedWith[(i << 1) + 1][(j << 1)] *
                             associatedWith[(i << 1)][(j << 1) + 1];
    }
  }
  ...
  for (int i = 0; i < initVarNum; ++i)
    selArray[i] = 0;
  for (int i = 0; i < selectors.size(); ++i)
    selArray[var(selectors[i]) - (initVarNum << 1)] = 1;
  for (int i = 0; i < initVarNum; ++i) {
...
*/
void DefinableDetection::addSelectedEquivalence(Solver &solver, vec<Var> &v1,
                                                vec<Var> &v2,
                                                vec<Lit> &selectors) {
...
*/
void DefinableDetection::addShiftFormula(Solver &solver, vec<vec<Lit>> &cls,
                                         int shift) {
  ...
  for (int i = 0; i < cls.size(); i++) {
    ...
    for (int j = 0; j < cl.size() && !isSAT; j++) {
      while (solver.nVars() <= (var(cl[j]) + shift))
        solver.newVar();
      if (solver.value(cl[j]) == l_Undef)
        phClause.push(mkLit(var(cl[j]) + shift, sign(cl[j])));
      else
        isSAT = solver.value(cl[j]) == l_True;
    }
...
*/
void DefinableDetection::computeBackBone(Solver &solver,
...
*/
void DefinableDetection::constructive(Solver &solver, vec<Lit> &assums,
                                      int nbVar, vec<Var> &i, vec<Var> &o,
                                      int limitC, vec<vec<Lit>> &gates) {
  vec<bool> isInput;
  ...
  while (assums.size()) {
    ...
    if (!isProtected[vc] && !definable) {
      ...
      for (int i = 0; !definable && i < occGates[vc].size(); i++) {
        ...
        definable = true;
        for (int j = 1; definable && j < g.size(); j++)
          definable = isInput[var(g[j])];
      }
    ...
    if (!definable) {
      ...
      for (int i = 0; i < occGates[vc].size(); i++)
        gates[occGates[vc][i]].clear();
      isInput[vc] = false;
      o.push(var(l) - (nbVar << 1));
      if (!solver.decisionLevel() && solver.value(l) == l_Undef)
        solver.uncheckedEnqueue(~l, CRef_Undef);
    }
  }
} // constructive
...
*/
void DefinableDetection::collectBiPartition(vec<vec<Lit>> &cls, int nbVar,
                                            vec<Var> &i, vec<Var> &o,
  ...
  if (!strcmp("NATURAL_ORDER", defSort))
    sortSelectorsDummy(selectors, assums);
  else if (!strcmp("OCC_ASC", defSort))
    sortSelectorsOccDesc(solver, selectors, assums, nbVar, cls);
  else
