...
void Forgetting::removeClauseOcc(int idx, vec<CRef> &refClauses)
{
  ...
  for(int i = 0 ; i<c.size() ; i++)
    {
      if(!markedOutputVars[var(c[i])]) continue;

      // printf("search %d in %d: ", idx, readableLit(c[i])); showListInt(occurrences[toInt(c[i])]);
...
void Forgetting::addClauseOcc(vec<Lit> &lits, vec<CRef> &refClauses)
{
  ...
  if(lits.size() == 1)
    ...
    {            
      ...
      else{refClauses.push(); hashKeyInit.push(); stampSubsum.push(0);}
  ...
  for(int i = 0 ; i<refClauses.size() ; i++)
    {
      ...
      for(int j = 0 ; j<c.size() ; j++)
        if(markedOutputVars[var(c[j])]) occurrences[toInt(c[j])].push(i);
    }  
}// initOccList
      ...
      for(int j = 0 ; j<c.size() && !isSAT ; j++)
        {
          ...
          if(solver.value(c[j]) != l_Undef) continue;
          else solver.uncheckedEnqueue(c[j] == l ? c[j] : ~c[j]);
        }

      if(isSAT) removeClauseOcc(v[i], refClauses); // we can remove the (satisfiable) clause
      else if(solver.propagate() != CRef_Undef)
        {
          ...
          if(c.size() > 2)
          ...
          else 
            {
              ...
              if(solver.value(c[1]) == l_Undef) solver.uncheckedEnqueue(c[1]);
...
void Forgetting::vivifiedSetOfClauses(vec<CRef> &refClauses)
{
  ...
  for(int i = 0 ; i<refClauses.size() ; i++)
    {
      ...
      for(int k = 0 ; k<c.size() ; k++)
        {
          if(solver.value(c[k]) == l_True){keepClause = false; break;}          
          if(solver.value(c[k]) == l_False)
            {
              if(markedOutputVars[var(c[k])]) searchAndRemoveOcc(occurrences[toInt(c[k])], i);
      ...
      if(!keepClause || c.size() <= 1)
        ...
        {
          solver.attachClause(cr);
          if(shorted) hashKeyInit[i] = hashClause(c);
        }
      
      if(c.size() == 1 && solver.value(c[0]) == l_Undef) solver.uncheckedEnqueue(c[0]);
    }
  ...
  for(int i = 0 ; i<refClauses.size() ; i++) if(!mustBeRm[i]) assert(solver.ca[refClauses[i]].attached());
...
void Forgetting::constructWatchList(vec< vec<Lit> > &newCls)
{
  ...
  for(int i = 0 ; i<newCls.size() ; i++)
    {
      // we want to watch the first literal (but we need to change to avoid the collisions
...
/**
   ...
   @param[out] newCls, the new clauses which contains only non subsubmed clauses in the end of the process
 */
void Forgetting::removeSubsum(vec< vec<Lit> > &newCls, vec<CRef> &refClauses)
{
  // create the hash tables
  vec<uint64_t> hashKeyNew; 
  for(int i = 0 ; i<newCls.size() ; i++) hashKeyNew.push(hashSetOfLit(newCls[i]));
  ...
  for(int i = 0 ; i<newCls.size() ; i++)
    {
      ...
      for(int j = 0 ; j<cnew.size() ; j++) markedLit[toInt(cnew[j])] = true;
      ...
      for(int j = 0 ; j<cnew.size() ; j++) markedLit[toInt(cnew[j])] = false;
 ...
 */
void Forgetting::generateAllResolution(Var v, vec< vec<Lit> > &cls, vec<CRef> &refClauses)
{
      ...
      for(int j = 0 ; j<cp.size() ; j++)
        {
          ...
          if(cp[j] != lPos && solver.value(cp[j]) != l_False) base.push(cp[j]);
        }
      ...
      for(int j = 0 ; j<idxNegLit.size() ; j++)
        {
          ...
          for(int k = 0 ; k<cn.size() && ! taut ; k++)
            {
              if(cn[k] == lNeg || solver.value(cn[k]) == l_False) continue;
...
*/
void Forgetting::extractEquivClass(vec< vec<Lit> > &equivList, vec<Lit> &equivClass)
{
  vec<int> stampChain;
  equivClass.clear();
  for(int i = 0 ; i<solver.nVars() ; i++){ equivClass.push(mkLit(i, false)); stampChain.push(0);}

  int stampChainIdx = 0;
  for(int i = 0 ; i<equivList.size() ; i++)
    {
      if(!equivList[i].size()) continue;

      stampChainIdx++;
      stampChain[var(equivList[i][0])] = stampChainIdx;

      vec<Var> chain;      
      Var varInput = markedOutputVars[var(equivList[i][0])] ? var_Undef : var(equivList[i][0]);
      ...
      while(findAtLeastOn)
        {
          ...
          for(int j = i ; j<equivList.size() ; j++)
            {
              if(!equivList[j].size()) continue;
              if(stampChain[var(equivList[j][0])] == stampChain[var(equivList[j][1])]) // already/not renamed
                {
                  if(stampChain[var(equivList[j][0])] == stampChainIdx) equivList[j].clear();
                  continue;
                }

              Lit lStamp = lit_Undef, lNoStamp = lit_Undef;
              if(stampChain[var(equivList[j][0])] != stampChainIdx) lNoStamp = equivList[j][0]; else lStamp = equivList[j][0];
              if(stampChain[var(equivList[j][1])] != stampChainIdx) lNoStamp = equivList[j][1]; else lStamp = equivList[j][1];
              assert(lStamp != lit_Undef && lNoStamp != lit_Undef);

              if(sign(lStamp)){lStamp = ~lStamp; lNoStamp = ~lNoStamp;} // adjust the sign
              if(sign(lNoStamp)) equivClass[var(lNoStamp)] = ~equivClass[var(lStamp)];
              else equivClass[var(lNoStamp)] = equivClass[var(lStamp)];

              // keep the input variable if it exists
              chain.push(var(lNoStamp));
              if(varInput == var_Undef) varInput = markedOutputVars[var(lNoStamp)] ? var_Undef : var(lNoStamp);

              findAtLeastOn = true;
              stampChain[var(equivList[j][0])] = stampChain[var(equivList[j][1])] = stampChainIdx;
              equivList[j].clear();
            }
        }
      ...
      if(varInput != var_Undef)
        {
          ...
          for(int i = 0 ; i<chain.size() ; i++)
            if(sign(equivClass[chain[i]])) equivClass[chain[i]] = ~pos; else equivClass[chain[i]] = pos;          
        }
    }
}// extractEquivClass
...
void Forgetting::replaceEquiv(vec< vec<Lit> > &gates)
{
  ...
  for(int i = 0 ; i<clauses.size() ; i++) solver.detachClause(clauses[i], true);
  ...
  for(i = j = 0 ; i<clauses.size() ; i++)
    {
      ...
      for(int k = 0 ; k<c.size() ; k++)
        {
          if(markedOutputVars[var(c[k])] && var(equivClass[var(c[k])]) != var(c[k]))
            {
              change = true;
              if(sign(c[k])) c[k] = ~equivClass[var(c[k])]; else c[k] = equivClass[var(c[k])];
            }
          ps.push(c[k]);
        }
      if(!change){clauses[j++] = clauses[i]; continue;}
      ...
      for (Lit p = lit_Undef; !isSAT && ki < ps.size(); ki++)
        {
          ...
          if(solver.value(ps[ki]) != l_False && ps[ki] != p) ps[kj++] = p = ps[ki];
        }
      ps.shrink(ki - kj);
      
      if(isSAT){c.mark(1); (solver.ca).free(clauses[i]);}
      else if(ps.size() == 0) killUnsat();
      else if(ps.size() == 1)
        {
          isSAT = true;
          if(solver.value(ps[0]) == l_False) killUnsat();
          if(solver.value(ps[0]) == l_Undef) solver.uncheckedEnqueue(ps[0]);
        }  
      else
        {      
          for(int k = 0 ; k<ps.size() ; k++) c[k] = ps[k];
  ...
  for(int i = 0 ; i<clauses.size() ; i++) solver.attachClause(clauses[i]);  
}// replaceEquiv
...
int Forgetting::computeAdditionalNumberOfClauses(vec<Lit> &orGate)
{
  ...
  for(int i = 1 ; i<orGate.size() ; i++) markedLit[var(orGate[i])] = sign(orGate[i]) + 1;
  ...
  for(int i = 0 ; i<v1.size() ; i++)
    {
      ...
      // printf("+ "); showClause(c);
      ret++;      
      for(int j = 0 ; j<c.size() ; j++) if(markedLit[var(c[j])] + sign(c[j]) == 2){ret--; break;}
    }
  // printf("+ ret = %d\n", ret);
  ...
  for(int i = 0 ; i<v2.size() ; i++)
    {
      ...
      for(int j = 0 ; j<c.size() ; j++)
        {
          if((markedLit[var(c[j])] + sign(c[j])) == 2) cpt--;
          if(markedLit[var(c[j])] && ((markedLit[var(c[j])] + sign(c[j])) & 1)){cpt = 1; break;}
        }
  ...
  for(int i = 1 ; i<orGate.size() ; i++) markedLit[var(orGate[i])] = 0;
...
void Forgetting::replaceLitByTerm(Lit l, vec<Lit> &lits)
{
  ...
  for(int i = 0 ; i<lits.size() ; i++) markedLit[var(lits[i])] = 1 + sign(lits[i]);
  ...
  for(int i = 0 ; i<nv.size() ; i++)
    {
      ...
      for(int j = 0 ; j<c.size() ; j++)
        {
          if(c[j] != l) addClause.push(c[j]);
          if(markedLit[var(c[j])]) markedLit[var(c[j])] += 1 + sign(c[j]); 

          // printf("==> %d = %d\n", readableLit(c[j]), markedLit[var(c[j])]);
          subsum = subsum || (markedLit[var(c[j])] && !(markedLit[var(c[j])] & 1)); // markedLit[var(c[j])] == 2 || 4 
        }
      ...
      if(subsum)
        {
          ...
          if(addClause.size() > 1)
            ...
            }else
            {
              ...
              if(solver.value(addClause[0]) == l_Undef) solver.uncheckedEnqueue(addClause[0]);
            }
        ...
        }else
        {
          ...
          for(int j = 0 ; j<lits.size() ; j++)
            {
              // printf("booo %d : %d\n", markedLit[var(lits[j])], readableLit(lits[j]));
      ...
      for(int j = 0 ; j<lits.size() ; j++) markedLit[var(lits[j])] = 1 + sign(lits[j]);
      // printf("reinit\n");
      // for(int j = 0 ; j<lits.size() ; j++) printf("%d ~~> %d\n", readableLit(lits[j]), markedLit[var(lits[j])]);
    }

  // unmark term
  for(int i = 0 ; i<lits.size() ; i++) markedLit[var(lits[i])] = 0;
...
{  
  ...
  for(int i = 0 ; i<lits.size() ; i++) markedLit[var(lits[i])] = sign(lits[i]) + 1; 
  ...
  for(int i = 0 ; i<pv.size() ; i++)
    {     
      ...
      for(int k = 0 ; k<lits.size() ; k++) assert(markedLit[var(lits[k])] == sign(lits[k]) + 1);
      ...
      for(int j = 0, end = c.size() ; !taut && j<end ; j++)
        {
          ...
          if(c[j] == l) continue;
          else if(!markedLit[var(c[j])]) lits.push(c[j]);
          else
            {
              taut = (markedLit[var(c[j])] + sign(c[j]) == 2) || solver.value(c[j]) == l_True;
              markedLit[var(c[j])] += 3;
            }
        }
      ...
      if(!taut && lits.size() > 1)
        {
          ...
          for(int j = 0 ; j<lits.size() ; j++)
            if(markedOutputVars[var(lits[j])] && markedLit[var(lits[j])] <= 3) occurrences[toInt(lits[j])].push(pv[i]);          
          solver.attachClause(solver.clauses[pv[i]]);
        }else
        {
          // printf("MUST BE REMOVED %d\n", pv[i]);
          
          if(!taut && lits.size() == 1 && solver.value(lits[i]) == l_Undef) solver.uncheckedEnqueue(lits[0]);

          for(int j = 0 ; j<c.size() ; j++)
            if(c[j] != l && markedOutputVars[var(c[j])]) searchAndRemoveOcc(occurrences[toInt(c[j])], pv[i]);
      ...
      for(int j = 0 ; j<lits.size() ; j++) markedLit[var(lits[j])] = sign(lits[j]) + 1;
...
{
  vec<Lit> lits;
  for(int i = 1 ; i<g.size() ; i++) lits.push(g[i]);

  // printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~> replaceVarByOrGate: "); showListLit(g);
  // debug();
  // printf("------------------\n");
 ...
 */
int Forgetting::runForgetting(vec<Var> &outputVars, vec<Var> &forgetVars, int lim_occ, vec< vec<Lit> > &gates, bool useGates)
{
  ...
  // we create the occurence list refer all the clause where an output variable exist
      ...
      while(outputVars.size())
        {
          ...
          // printf("~~~~~~~  %d  ~~~~~~~~~~\n", varSelected + 1);
          if(solver.value(varSelected) != l_Undef) continue;
          ...
          // gate option
          if(useGates) tryGateElimination(lp, gd);
          
          if(occurrences[toInt(lp)].size() * occurrences[toInt(~lp)].size() > lim_occ){notAlreadyForget.push(varSelected); continue;}
          
          vec< vec<Lit> > genResolution;
          generateAllResolution(varSelected, genResolution, refClauses);
          removeSubsum(genResolution, refClauses);
      
          int sureRm = occurrences[toInt(lp)].size() + occurrences[toInt(~lp)].size();
          if(genResolution.size() <= sureRm)
            {
              ...
              forgetApplied = true;
              vec<int> idxRm;      
              for(int j = 0 ; j<occurrences[toInt(lp)].size() ; j++) idxRm.push(occurrences[toInt(lp)][j]);
              for(int j = 0 ; j<occurrences[toInt(~lp)].size() ; j++) idxRm.push(occurrences[toInt(~lp)][j]);
          
              for(int j = 0 ; j<idxRm.size() ; j++) removeClauseOcc(idxRm[j], refClauses);      
              for(int j = 0 ; j<genResolution.size() ; j++) addClauseOcc(genResolution[j], refClauses);
  ...
  int i, j;
  for(i = j = 0 ; i<refClauses.size() ; i++) if(!mustBeRm[i]) refClauses[j++] = refClauses[i];
