...
namespace Glucose
{
 ...
 protected:
    struct LitOrderLt
    {
      ...
      {
        if(markedOutputVars[var(x)] && !markedOutputVars[var(y)]) return true;
        if(!markedOutputVars[var(x)] && markedOutputVars[var(y)]) return false;
        return activity[var(x)] > activity[var(y)];
      }
    LitOrderLt(const vec<double>&  act, const vec<bool> &m) : activity(act), markedOutputVars(m) { }
    };
  ...
  private:            
    ...
    inline void showOccurrenceList()
    {
      for(int i = 0 ; i<solver.nVars() ; i++)
        {
          if(!markedOutputVars[i]) continue;
          Lit l = mkLit(i, true);
          
          printf("%d: ", readableLit(l)); showListInt(occurrences[toInt(l)]);
          printf("%d: ", readableLit(~l)); showListInt(occurrences[toInt(~l)]);
        }
    }
      ...
      for(int i = 0 ; i<solver.clauses.size() ; i++)
        {
          ...
          printf("debug %d: ", c.attached()); showClause(c);
          if(!c.attached()) continue;

          for(int j = 0 ; j<c.size() ; j++)
            {
              if(!markedOutputVars[var(c[j])]) continue;
          ...
          for(int j = 0 ; j<occurrences[toInt(l)].size() ; j++)
            {
              ...
              printf("&&& %d: ", readableLit(l)); showClause(c);
              assert(c.attached());
    ...
    inline void extractEquivFromGates(vec< vec<Lit> > &gates, vec< vec<Lit> > &equivList)
    {
      int i, j;
      for(i = j = 0 ; i<gates.size() ; i++)
        {
          if(gates[i].size() > 2 && i == j) j++;
    ...
    {
      int best = 0, score = occurrences[toInt(mkLit(outputVars[0], false))].size()* occurrences[toInt(mkLit(outputVars[0], true))].size();
      
      for(int i = 1 ; i<outputVars.size() ; i++)
        {
          if(solver.value(outputVars[i]) != l_Undef) continue;
          
          Lit l = mkLit(outputVars[i], false);
          if(!occurrences[toInt(l)].size() || !occurrences[toInt(~l)].size()){ best = i; break;}
    ...
    inline void tryGateElimination(Lit lp, GatesDetection &gd)
    {
      ...
      if(pg.size() && !ng.size()){if(computeAdditionalNumberOfClauses(pg) <= lim) replaceVarByOrGate(pg);}
      else if(ng.size() && !pg.size()){if(computeAdditionalNumberOfClauses(ng) <= lim) replaceVarByOrGate(ng);}
      if(ng.size() && pg.size())
        {
          int nb = computeAdditionalNumberOfClauses(ng), pb = computeAdditionalNumberOfClauses(pg);          
          if(nb <= lim || pb <= lim)
            {
              if(nb < pb) replaceVarByOrGate(ng);
              else replaceVarByOrGate(pg);
            }
        }
    }// tryGateElimination
  ...
  Forgetting(Solver &s) : solver(s)
    {
      timeOccRm = nbRemovedLitOccRm = timeVivi = nbRemovedClauseVivi = nbRemovedLitVivi = 0;      
      markedLit.initialize(s.nVars() << 1, false);
      stamp = 0;
      for(int i = 0 ; i<solver.nVars() ; i++){markedAsForget.push(false); watchNewCls.push(); watchNewCls.push(); tmpVector.push(0);}
    }
    
    int runForgetting(vec<Var> &outputVars, vec<Var> &forgetVar, int lim_occ, vec< vec<Lit> > &gates, bool useGate);
  };
}
